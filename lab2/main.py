...
def train_epoch(model, dataloader, criterion: dict, optimizer,
    ...
    for X, y in bar:
        ...
        img = X.to(device)
        label, bbox = y[0].to(device), y[1].to(device)
        cls, bbox_pred = model(img)

        cls_loss = criterion['cls'](cls, label)
        bbox_loss = criterion['box'](bbox_pred, bbox)
        loss = cls_loss + bbox_loss
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()

        cls_num = (torch.argmax(cls, axis=1) == label).cpu().detach().numpy()
        iou_thres = compute_iou(bbox_pred.cpu().detach().numpy(), bbox.cpu().cpu().detach().numpy()) > iou_thr
        correct += sum(cls_num * iou_thres)
        total += img.shape[0]
        ...
